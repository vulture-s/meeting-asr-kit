...
多人、多語、單點收音的會議錄音，**怎麼做出一份你敢用的逐字稿**。

這不是又一個 Whisper 包裝。轉錄誰都會跑 —— 難的是那份稿子送到你面前時，
你不知道它漏了什麼、哪裡在猜。這個 repo 是一條**聽打流程**，
加上一件事：**流程本身是被量過的**。
...
`transcribe_ui.py` 讓那一點點變便宜；其餘的工具在回答
「所以自動轉的那一大半，到底有多準」。
...
而任何網頁都能 POST 一個空陣列把整份稿子清掉。兩個都修了，
